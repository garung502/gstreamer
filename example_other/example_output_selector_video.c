...
static void on_bin_element_added (GstBin * bin, GstElement * element, gpointer user_data)
{
  gchar *name;
  g_object_get (G_OBJECT (element), "name", &name, NULL);
  g_print ("ZZZZZZZZZZZZZZZZZZZZZZZZZZZZ The name of the element is '%s'.\n", name);
  g_free (name);
...
gint main (gint argc, gchar * argv[])
{
  ...
  //g_signal_connect (G_OBJECT (sink1), "element-added", G_CALLBACK (on_bin_element_added), NULL);
  //g_signal_connect (G_OBJECT (sink2), "element-added", G_CALLBACK (on_bin_element_added), NULL);

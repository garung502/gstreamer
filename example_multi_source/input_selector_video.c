...
switch_timer (GstElement * video_switch)
{
 ...
 else if (strcmp (active_name, "sink_0") == 1) {
    new_pad = gst_element_get_static_pad (video_switch, "sink_2");
 }
 else {
...
main (int argc, char *argv[])
{
  ...
  g_object_set (G_OBJECT (src3), "pattern", 2, NULL);
  ...
  sink3_sync = gst_element_factory_make ("identity", "sink2_sync");
  g_object_set (G_OBJECT (sink3_sync), "sync", TRUE, NULL);

  gst_bin_add_many (GST_BIN (pipeline), src1, src2, src3, segment, video_switch,
      video_sink, sink1_sync, sink2_sync, sink3_sync, capsfilter, NULL);
  ...
  gst_element_link (src3, sink3_sync);
  gst_element_link (sink3_sync, video_switch);
  ...
  g_timeout_add (1000, (GSourceFunc) switch_timer, video_switch);
  ...
  // debug 
  GST_DEBUG_BIN_TO_DOT_FILE(pipeline, GST_DEBUG_GRAPH_SHOW_ALL, "pipeline_input_selector");

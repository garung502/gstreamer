...
G_GNUC_UNUSED static GstCaps* gst_play_type_find (GstBin *bin, GstElement *element)
{
    GstElement *typefind;
    GstElement *pipeline;
    GstCaps *caps = NULL;

    GST_DEBUG (0,"GstPipeline: typefind for element \"%s\"", GST_ELEMENT_NAME(element));
    ...
    gst_pad_link (gst_element_get_static_pad (element, "src"), gst_element_get_static_pad (typefind, "sink"));
    ...
    caps = gst_pad_get_current_caps (gst_element_get_static_pad (element, "src"));

    gst_pad_unlink (gst_element_get_static_pad (element, "src"), gst_element_get_static_pad (typefind, "sink"));
...
int main(int argc,char *argv[]) 
{
    ...
    gst_pad_link(gst_element_get_static_pad(adder,"src"), gst_element_get_static_pad(audiosink,"sink"));
    ...
    for (i = 1; i < argc; ++i) {
        ...
        gst_pad_link (gst_element_get_static_pad (channel_in->pipe, buffer), pad);
...
input_channel_t* create_input_channel (int id, char* location)
{
    ...
    */
    ...
    GST_DEBUG (0, "c_i_p : creating channel with id %d for file %s", id, location);
...
if (!srccaps) {
    ...
    new_element = gst_autoplug_to_caps (autoplug, srccaps, gst_caps_new ("audio", "audio/raw", NULL), NULL);
    ...
    //gst_element_add_ghost_pad (new_element, gst_element_get_static_pad (decoder, "sink"), "sink");
    //gst_element_add_ghost_pad (new_element, gst_element_get_static_pad (decoder, "src"), "src_00");
                                                                                      
    gst_element_add_pad (new_element, gst_element_get_static_pad (decoder, "sink"), "sink");
    gst_element_add_pad (new_element, gst_element_get_static_pad (decoder, "src"), "src_00");
#endif  
#ifndef GST_DISABLE_LOADSAVE
    ...
    //gst_element_add_ghost_pad (channel->pipe, gst_element_get_static_pad (channel->volenv, "src"), buffer);

    gst_element_add_pad (channel->pipe, gst_element_get_static_pad (channel->volenv, "src"), buffer);
...
void destroy_input_channel (input_channel_t *channel)
{
    /* 

...
static void value_changed_callback (GtkWidget * widget, gpointer * user_data)
{
  ...
  g_print("XXXXXXXXXXXXXXXXXXXXXXXXX GIA TRI CUA VOLUME :%f \n",value); 
  g_object_set (volumes[0], "volume", value, NULL);
  
  g_object_set (volumes[1], "volume", value, NULL);

  g_object_set (volumes[2], "volume", value, NULL);

}

static void setup_gui (GstElement * volume, gchar * file_name1, gchar * file_name2)
{
...
static void eos_message_received (GstBus * bus, GstMessage * message, GstPipeline * pipeline)
{
  message_received (bus, message, pipeline);
  gtk_main_quit ();
}

static void dynamic_link (GstPadTemplate * templ, GstPad * newpad, gpointer user_data)
{
...
static void make_mixer_channel (GstElement * pipeline, GstElement * mix, gchar * file_name)
{
...
int main (int argc, char *argv[])
{
  ...
  GstElement *audio_enc, *file_sink;
  GstBus *bus;
  /*
  if (argc < 3) {
  ...
  */
  if (argc < 4) {
    g_print ("Usage: audiomix <file1> <file2> <file3> \n");
    return 1;
  }
  ...
  //mix = gst_element_factory_make ("GstAudioMixer", NULL);
  ...
  //gst_element_link_many (mix, convert, sink, NULL);
   ...
   // begin
  // test flush to recorder file
  audio_enc = gst_element_factory_make("wavenc", NULL);
  g_assert(audio_enc);

  file_sink = gst_element_factory_make("filesink", NULL);
  g_assert(file_sink);
  g_object_set(file_sink, "location", "file_recorder.wav", NULL);
  
  gst_bin_add_many (GST_BIN (pipeline), mix, convert, audio_enc, file_sink, NULL);
  gst_element_link_many (mix, convert, audio_enc, file_sink, NULL);
  // end 

  /* prepare mixer channel strips */
  make_mixer_channel (pipeline, mix, argv[1]);
  make_mixer_channel (pipeline, mix, argv[2]);
  make_mixer_channel (pipeline ,mix, argv[3]);

  /* setup message handling */
  bus = gst_pipeline_get_bus (GST_PIPELINE (pipeline));
  gst_bus_add_signal_watch_full (bus, G_PRIORITY_HIGH);
  g_signal_connect (bus, "message::error", (GCallback) message_received, pipeline);

  g_signal_connect (bus, "message::warning", (GCallback) message_received, pipeline);

...
int main(int argc, char *argv[]) {
  ...
  for (walk = list; walk != NULL; walk = g_list_next (walk)) {
    ...
    g_print("ZZZZZZZZZZZZZZZ:  %s\n", name);

    //if (selected_factory == NULL || g_str_has_prefix (name, "GOOM")) {
    if (selected_factory == NULL || g_str_has_prefix (name, "Stereo")) {
  ...
  pipeline = gst_parse_launch ("playbin uri=file:///home/truongtv/bt/gstreamer/ghita.flac", NULL);
